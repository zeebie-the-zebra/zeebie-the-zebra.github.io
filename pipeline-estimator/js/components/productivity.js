...
function render(container) {
    ...
    <div class="card">
    <div class="card-body">
    <!-- ... Productivity Factors table (unchanged) ... -->
    <div class="d-flex justify-content-between">
    <h5 class="card-title">Productivity Factors</h5>
    <div class="alert alert-warning p-2 small"><strong>Please be careful adjusting this section:</strong> This should only need adjusting if work is undertaken outside of the work shop.</div>
    </div>
    <div class="row mt-3">
    <div class="col-md-8">
    <table class="table table-sm table-bordered" id="productivity-items-table">
    <thead class="table-light"><tr><th>Productivity Items</th><th class="text-center">Productivity %</th></tr></thead>
    <tbody>
    ...
    <div class="col-md-4"><table class="table table-sm table-bordered"><thead class="table-light"><tr><th>Type</th><th class="text-center">% Range</th></tr></thead><tbody><tr><td>Very Low</td><td class="text-center">10 - 40</td></tr><tr><td>Low</td><td class="text-center">41 - 60</td></tr><tr><td>Average</td><td class="text-center">61 - 80</td></tr><tr><td>Very Good</td><td class="text-center">81 - 90</td></tr><tr><td>Excellent</td><td class="text-center">91 - 100</td></tr></tbody></table></div>
    </div>
    <table class="table table-sm table-bordered" style="max-width: 400px;">
    <tbody>
    <tr><td class="fw-bold" data-bs-toggle="tooltip" title="Total Job Productivity; Anything over 70% is considered near perfect and as such labour rates do not get adjusted. If value drops below 70% labour rates will be increased for the company to cover the % of costs lost for the reduction in productivity.">Total Job Productivity</td><td class="text-center">${totals.totalJobProductivity.toFixed(2)}%</td></tr>
    <tr><td>Lost Labour Productivity</td><td class="text-center">${totals.lostLabourProductivity.toFixed(2)}%</td></tr>
    <tr><td colspan="2" class="text-center ${totals.adjustmentStatus.includes('wont') ? 'text-success' : 'text-danger'} fw-bold">${totals.adjustmentStatus}</td></tr>
    </tbody>
    </table>
    </div>
    </div>
...
function addEventListeners(container) {
    container.addEventListener('change', (event) => {
        ...
        } else if (target.id === 'tax-rate') { // <-- Listen for changes to tax rate
...
export default {
    init(container, data) {
        productivityData = {
            items: { ...defaultData.items, ...(data?.items || {}) },
            standardRates: { ...defaultData.standardRates, ...(data?.standardRates || {}) },
            taxRate: data?.taxRate ?? defaultData.taxRate, // Use saved or default
